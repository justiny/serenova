...
var NelioABEditExperiment = {
	...
	init: function() {
		jQuery('#exp_id').val(nelioabBasicInfo.id);
		jQuery('#exp_name').val(nelioabBasicInfo.name);
		jQuery('#exp_descr').val(nelioabBasicInfo.description);
	...
	save: function(action) {
		...
		smoothTransitions();
		jQuery('.nelio-exp-form > #action').val(action);
		jQuery.post(
			location.href,
...
var NelioABGoalCards = {
	...
	init: function() {
	   ...
	   var goals = JSON.parse( jQuery('#nelioab_goals').val() );
	...
	createCard: function( id ) {
		...
		newCard.find('h3 .form .rename').click(function() {
			if ( !NelioABGoalCards.validateGoalName( newCard ) )
				return;
			var newName = newCard.find('.new-name').first().val();
		...
		newCard.find('h3 .name .rename a').click(function() {
			...
			newCard.find('.new-name').first().val( oldName );
	...
	addAction: function(action, card) {
		...
		switch ( action.type ) {
			...
			case 'page':
				...
				if ( action.isNew !== true ) {
					if ( action.is_indirect )
						result.find('.direct').val( '0' );
					else
						result.find('.direct').val( '1' );
					searcher.val( action.value );
			...
			case 'post':
				...
				searcher.removeAttr('id');
				searcher.removeAttr('name');
				NelioABPostSearcher.buildSearcher( searcher, 'nelioab-all-cpt', 'show-drafts', NelioABGoalCards.filterPosts );
				result.show();
				break;
			...
			case 'external-page':
				...
				if ( action.isNew !== true ) {
					result.find('.name').val( action.name );
					result.find('.url').val( action.url );
					result.find('.url_mode').val( action.url_mode );
					if ( action.is_indirect )
						result.find('.direct').val( '0' );
					else
						result.find('.direct').val( '1' );
			...
			case 'click-element':
				...
				if ( action.isNew !== true ) {
					result.find('.mode').val( action.mode );
					result.find('.value').val( action.value );
				}
			...
			case 'form-submit':
				...
				if ( action.isNew !== true ) {
					searcher.val(action.form_id);
					if ( action.any_page )
						result.find('.any-page').val( '1' );
					else
						result.find('.any-page').val( '0' );
	...
	filterPosts: function( elem, data ) {
		var idsToIgnore = [];
		var aux = jQuery( '#exp_original' ).val();
		...
		card.find( 'input.post-searcher' ).each(function() {
			...
			idsToIgnore.push( parseInt( jQuery( this ).val() ) );
		});
		var result = [];
	...
	validatePageOrPost: function(action) {
		var ok = true;
		if ( action.find('input.post-searcher').val() == '' )
			ok = false;
		if ( ok ) {
	...
	validateForm: function(action) {
		var ok = true;
		if ( action.find('input.form-searcher').val() == '' )
			ok = false;
		if ( ok ) {
			action.find('.select2-container > a.select2-choice').removeClass('error');
			NelioABEditExperiment.manageProgress(true, true);
		...
		if ( result ) {
			card.find('.actions .action.external-page').each(function() {
				if ( action[0] != jQuery(this)[0] ) {
					...
					var offendedActionValue = offendedAction.val().trim();
	...
	validateClickElement: function(action, preventRecursive) {
		if (preventRecursive == undefined) preventRecursive = false;
		var card = action.closest('.nelio-card');
		var mode = action.find('select.mode').val();
		...
		if ( result ) {
			card.find('.actions .action.click-element').each(function() {
				if ( action[0] != jQuery(this)[0] ) {
					var offendedMode = jQuery(this).find('select.mode').val();
					var offendedInput = jQuery(this).find('input.value');
					var offendedValue = offendedInput.val().trim();
	...
	save: function() {
		...
		cards.each(function() {
			...
			var actions = card.find('.actions .action').each(function() {
				...
				switch( action.data( 'type' ) ) {
					...
					case 'page':
					case 'post':
						a.value = action.find( 'input.post-searcher.' + action.data( 'type' ) ).val();
						a.is_indirect = action.find( '.direct' ).val() == '0';
						if ( a.value == '' )
							break;
						g.actions.push(a);
						break;
					...
					case 'external-page':
						a.name = action.find( '.name' ).val();
						a.url = action.find( '.url' ).val();
						a.url_mode = action.find( '.url_mode' ).val();
						a.is_indirect = action.find( '.direct' ).val() == '0';
						if ( a.name == '' || a.url == '' )
							break;
						g.actions.push(a);
					...
					case 'wc-order-completed':
						a.value = action.find( 'input.post-searcher.product' ).val();
						if ( a.value == '' )
							break;
						g.actions.push(a);
						break;
					...
					case 'click-element':
						a.mode = action.find( '.mode' ).val();
						a.value = action.find( '.value' ).val();
						if ( a.mode == '' || a.value == '' )
							break;
						g.actions.push(a);
						break;
					...
					case 'form-submit':
						if ( action.hasClass( 'cf7' ) )
							...
							break;
						a.form_id = action.find( 'input.form-searcher' ).val();
						a.any_page = action.find( '.any-page' ).val() == '1';
						g.actions.push(a);
						break;
		...
		jQuery('#nelioab_goals').val( JSON.stringify( NelioABGoalCards.goals ) );
	},
