...
var NelioABAltTable = {
	...
	init: function(alts) {
		...
		table.on( 'new-form-shown', function( ev, editor ) {
			...
			editor.find('#qe_alt_name').val(name);
	...
	create: function() {
		...
		var newName = jQuery('#new_alt_name').val();
		var copyFrom = jQuery('#based_on').val();
	...
	showNewPageOrPostAltForm: function(table, copyingContent) {
		if ( copyingContent ) {
			...
			table.find('#based_on').val( -1 );
		}
		table.find('.new-alt-form').show();
		if ( table.find('#qe_alt_name').val().trim() == 0 )
	...
	save: function() {
		jQuery('#nelioab_alternatives').val( JSON.stringify( NelioABAltTable.alts ) );
	},
