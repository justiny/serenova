	...
	heatmap.prototype = {
		...
		drawAlpha: function(x, y, count, colorize){
				...
				var me = this,
					...
					max = me.get("max"),
					bounds = me.get("bounds"),
					xb = x - (2.5 * radius) >> 0, yb = y - (2.5 * radius) >> 0,
					xc = x + (2.5 * radius) >> 0, yc = y + (2.5 * radius) >> 0;
...
var nelioabHeatmapCss = 'body > canvas:last-child{ background:rgba(0,0,0,0.4); opacity:1; }';
...
(function() {
	...
	style.type = 'text/css';
	if (style.styleSheet)
		style.styleSheet.cssText = nelioabHeatmapCss;
	else
		style.appendChild(document.createTextNode(nelioabHeatmapCss));
...
function nelioabModifyHeatmapOpacity(opacity) {
	if ( opacity <= 0 ) opacity = 1;
	if ( opacity > 10 ) opacity = 10;

	nelioabHeatmapCss = 'body > canvas:last-child{ ';
	if ( opacity == 1 ) {
		nelioabHeatmapCss += 'background:rgba(0,0,0,0); opacity:0.64; }';
	}
	else if ( opacity == 2 ) {
		nelioabHeatmapCss += 'background:rgba(0,0,0,0); opacity:0.68; }';
	}
	else if ( opacity == 3 ) {
		nelioabHeatmapCss += 'background:rgba(0,0,0,0.05); opacity:0.72; }';
	}
	else if ( opacity == 4 ) {
		nelioabHeatmapCss += 'background:rgba(0,0,0,0.1); opacity:0.76; }';
	}
	else if ( opacity == 5 ) {
		nelioabHeatmapCss += 'background:rgba(0,0,0,0.15); opacity:0.8; }';
	}
	else if ( opacity == 6 ) {
		nelioabHeatmapCss += 'background:rgba(0,0,0,0.2); opacity:0.84; }';
	}
	else if ( opacity == 7 ) {
		nelioabHeatmapCss += 'background:rgba(0,0,0,0.25); opacity:0.88; }';
	}
	else if ( opacity == 8 ) {
		nelioabHeatmapCss += 'background:rgba(0,0,0,0.3); opacity:0.92; }';
	}
	else if ( opacity == 9 ) {
		nelioabHeatmapCss += 'background:rgba(0,0,0,0.35); opacity:0.96; }';
	}
	else {
		nelioabHeatmapCss += 'background:rgba(0,0,0,0.4); opacity:1.0; }';
	}

	var style = jQuery("#heatmap-style");
	if (style.length > 0) {
		style.text(nelioabHeatmapCss);
	}
