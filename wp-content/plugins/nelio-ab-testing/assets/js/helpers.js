...
NelioAB.helpers.remotePost = function( obj ) {
	...
	if ( NelioAB.helpers.isOldIE() ) {
		obj.data = {
			action: 'nelioab_forward_request_to_backend',
			backendMethod: obj.url.replace( NelioAB.backend.url, '' ),
			backendTextOptions: jQuery.param( obj.data )
		};
		obj.url = NelioABParams.ajaxurl;
	}
	jQuery.ajax( obj );
};
...
NelioAB.helpers.isNavigationRelevant = function() {
	...
	var i;
	for ( i = 0; i < NelioABEnv.rids.length; ++i )
		if ( NelioABEnv.rids[i] == NelioABParams.info.currentId )
			return true;

	for ( i = 0; i < NelioABEnv.goals.regularNavigationIds.length; ++i )
...
NelioAB.helpers.track = function() {
	...
	if ( ! nabTrackCurrentPage( NelioABParams.info.currentId ) ) {
		return;
	}//end if
			...
			switch ( ce.mode ) {
				...
				case 'css-path':
					...
					if ( ! found ) {
						var aux = target[0];
						var j = 0;
						while ( aux.parentNode && j < 100 ) {
							aux = aux.parentNode;
							++j;
						}
						try {
							if ( ! aux.parentNode && aux.nodeName !== '#document' ) {
								var tmp = document.createElement( 'div' );
								tmp.appendChild( aux );
								found = tmp.querySelectorAll( ce.value ).length > 0;
								tmp.removeChild( aux );
							}
						} catch ( exception ) {}
					}
				...
				case 'text-is':
					...
					for ( var j = 0; j < 5 && !successfulClick; ++j ) {
						...
						var currentValue = aux.text().replace(/\n/g,' ').replace(/\s+/g,' ').trim().toLowerCase();
...
if ( typeof nabTrackCurrentPage !== 'function' ) {

	nabTrackCurrentPage = function( id ) {
		return true;
	}//end nabTrackCurrentPage()
