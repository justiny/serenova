...
class NelioABAlternativeExperimentController {
	...
	public function hook_to_wordpress() {
		...
		add_filter( 'get_the_date',  array( &$this, 'use_originals_date' ), 10, 3 );
		add_filter( 'get_the_modified_date', array( &$this, 'use_originals_modified_date' ), 10, 3 );
		...
		add_filter( 'body_class', array( &$this, 'add_home_class_in_home_alternatives' ), 1, 99 );
		...
		add_filter( 'get_post_metadata', array( &$this, 'get_consistent_featured_image' ), 10, 3 );
		add_filter( 'get_post_status',   array( &$this, 'get_consistent_status' ), 10, 2 );
		...
		add_filter( 'get_post_metadata', array( &$this, 'fix_headline_featured_image' ), 10, 3 );
	...
	public function get_consistent_title( $title, $id = NULL ) {
		...
		if ( $menu_related_post ) {
			...
			if ( empty( $menu->post_title ) ) {
				...
				if ( ! is_wp_error( $post ) && ! empty( $post ) && $post->post_title == $menu->post_title )
					$id = $menu_related_post;
			}
		}
	 ...
	 * @since PHPDOC
	 */
	public function get_consistent_featured_image( $value, $object_id, $meta_key ) {

		if ( '_thumbnail_id' == $meta_key ) {
			remove_filter( 'get_post_metadata', array( &$this, 'get_consistent_featured_image' ), 10 );

			$alt_id = $this->get_post_alternative( $object_id );
			if ( $alt_id !== $object_id ) {
				$value = get_post_meta( $alt_id, $meta_key, true );
			}//end if
	...
	}//end get_consistent_featured_image()
	 ...
	 * @since PHPDOC
	 */
	public function get_consistent_status( $status, $post ) {

		if ( ! $this->is_post_alternative( $post->ID ) ) {
			return $status;
		}//end if

		$original = $this->get_original_related_to( $post->ID );
		return get_post_status( $original );

	}//end get_consistent_status()
	 ...
	 * @since PHPDOC
	 */
	public function load_proper_page_template( $value, $post_id, $meta_key ) {
		...
		if ( $meta_key === '_wp_page_template' ) {
			...
			if ( $this->is_post_in_a_post_alt_exp( $post_id ) ) {
				...
				$new_value = get_post_meta( $post_id, '_wp_page_template', true );
	...
	public function show_the_appropriate_widgets( $all_widgets ) {
		...
		if ( ! apply_filters( 'nelioab_enable_widget_experiments', true ) ) {
			remove_filter( 'sidebars_widgets', array( &$this, 'show_the_appropriate_widgets' ) );
			return $all_widgets;
		}//end if
	 ...
	 * @since PHPDOC
	 */
	public function use_originals_date( $date, $format, $post = false ) {
		...
		if ( $post_id ) {
			remove_filter( 'get_the_date', array( &$this, 'use_originals_date' ), 10, 3 );
			$date = get_the_date( $format, $this->get_original_related_to( $post_id ) );
			add_filter( 'get_the_date', array( &$this, 'use_originals_date' ), 10, 3 );
		}//end if
	 ...
	 * @since PHPDOC
	 */
	public function use_originals_modified_date( $date, $format, $post = false ) {
		...
		if ( $post_id ) {
			remove_filter( 'get_the_modified_date', array( &$this, 'use_originals_modified_date' ), 10, 3 );
			$date = get_the_modified_date( $format, $this->get_original_related_to( $post_id ) );
			add_filter( 'get_the_modified_date', array( &$this, 'use_originals_modified_date' ), 10, 3 );
		}//end if
	...
	public function prepare_comments_form( $comments ) {
		...
		(function($) {
			$(document).ready(function(){
				$("#comment_post_ID").attr( 'value', <?php echo json_encode( $ori_id ); ?> );
			});
		})(jQuery);
		</script><?php
	...
	public function get_post( $post_id ) {
		if ( ! isset( $this->cached_posts[ $post_id ] ) ) {
			$this->cached_posts[ $post_id ] = get_post( $post_id );
		}//end if
	...
	public function preview_css() {
		if ( ! nelioab_can_user_manage_plugin() )
			return;
		$css = get_option( 'nelioab_temp_css', false );
	 ...
	 * @since PHPDOC
	 */
	public function fix_headline_featured_image( $value, $object_id, $meta_key ) {

		if ( '_thumbnail_id' == $meta_key ) {
			remove_filter( 'get_post_metadata', array( &$this, 'fix_headline_featured_image' ), 10 );
	...
	}//end fix_headline_featured_image()
	...
	private function send_form_action_if_required( $type, $form_id ) {
		...
		$current_id = false;
		if ( isset( $_POST['nelioab_current_id'] ) ) {
			$current_id = absint( $_POST['nelioab_current_id'] );
		}//end if

		$current_actual_id = false;
		if ( isset( $_POST['nelioab_current_actual_id'] ) ) {
			$current_actual_id = absint( $_POST['nelioab_current_actual_id'] );
		}//end if

		$user_id = false;
		if ( isset( $_POST['nelioab_userid'] ) ) {
			$user_id = sanitize_text_field( $_POST['nelioab_userid'] );
		}//end if

		if ( empty( $current_id ) || empty( $current_actual_id ) || empty( $user_id ) ) {
			return;
		}//end if
	...
	public function add_disqus_fix() {
		if ( is_single() || is_page() ) {
			...
			$url = remove_query_arg( array( 'nab', 'nabe', 'nabc', 'nabm', 'nabt', 'nabw' ), $url );
			...
			<script type="text/javascript">
				var disqus_config = function() {
					...
					if ( $include_identifier ) { ?>
						this.page.identifier = '<?php echo json_encode( $post_id ); ?>';
					<?php
					} ?>
				};
			</script>
	 ...
	 * @since 4.3.1
	 */
	public function add_home_class_in_home_alternatives( $classes ) {
	...
	}//end add_home_class_in_home_alternatives()
