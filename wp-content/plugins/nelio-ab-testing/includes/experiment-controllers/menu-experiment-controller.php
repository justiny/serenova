	...
	class NelioABMenuExpAdminController {
		...
		public function __construct() {
			...
			if ( 'nav-menus.php' == $pagenow && $menu ) {
				...
				if ( $exp_id ) {
					...
					if ( md5( "$exp_id$menu" ) !== $check ) {
						wp_die( __( 'You do not have sufficient permissions to access this page.' ) );
					} else {
						add_action( 'in_admin_footer', array( $this, 'add_code_to_hide_menus_when_editing_menu_exp' ) );
					}//end if
			...
			add_filter( 'get_user_option_nav_menu_recently_edited', array( &$this, 'hide_recently_edited_menu_if_alternative' ) );
		...
		public function add_code_to_hide_menus_when_editing_menu_exp() {
			...
			<div id="nelioab-edit-alt-menus" class="widgets-holder-wrap" style="margin-bottom:1em;display:none;border: 1px solid #e5e5e5;-webkit-box-shadow: 0 1px 1px rgba(0,0,0,.04);box-shadow: 0 1px 1px rgba(0,0,0,.04);">
				<div class="widgets-sortables ui-sortable">
					<div class="sidebar-name" style="<?php echo esc_attr( 'padding:0.2em 1em;background-color:' . $colorscheme['focus'] ); ?>">
						...
						<h3 style="<?php echo esc_attr( 'color:' . $colorscheme['foreground'] ); ?>"><?php esc_html_e( 'Alternative Menu', 'nelio-ab-testing' ); ?></h3>
					</div>
				</div>
				...
				<div class="content" style="padding:0em 1em 1em 1em;">
					...
					<ul style="margin-left:1.5em;">
						<?php if ( isset( $_GET['back_to_edit'] ) ):
							$url = admin_url( 'admin.php?page=nelioab-experiments&action=edit&id=' . absint( $_GET['nelioab_exp'] ) . '&ctab=tab-alts&exp_type=' . NelioABExperiment::MENU_ALT_EXP );
							?>
							<li><a href="<?php echo $url; ?>"><?php esc_html_e( 'Editing this experiment', 'nelio-ab-testing' ); ?></a></li>
						<?php else:
							$url = admin_url( 'admin.php?page=nelioab-experiments&action=progress&id=' . absint( $_GET['nelioab_exp'] ) . '&exp_type=' . NelioABExperiment::MENU_ALT_EXP );
							?>
			...
			(function($) {
				...
				$aux.show();
				var $msg = $('#message');
				$msg.html($msg.html().replace(/Menu[0-9.\s]+(\s)?/, '<?php echo esc_html( __( 'Alternative Menu', 'nelio-ab-testing' ) ); ?>\1'));
			})(jQuery);
			</script>
			<?php
		}
		...
		public function duplicate_menu_and_create_alternative( $ori, $exp ) {
			...
			if ( !$new_id )
				return false;
			$ori = intval( $ori );
			remove_action( 'wp_update_nav_menu_item', 'nectar_nav_button_style_update', 10 );
