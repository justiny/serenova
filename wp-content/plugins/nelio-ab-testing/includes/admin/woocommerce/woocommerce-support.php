	...
	class NelioABWooCommerceSupport {
		...
		public function hook() {
			...
			add_filter( 'get_post_metadata',
				array( &$this, 'fix_product_summary_featured_image' ), 10, 3 );
		...
		public function save_visitor_information_in_order( $order_id ) {
			...
			if ( isset( $_POST['nelioab_userid'] ) ) {
				$value = sanitize_text_field( $_POST['nelioab_userid'] );
				update_post_meta( $order_id, '_nelioab_userid', $value );
			}//end if

			if ( isset( $_POST['nelioab_form_env'] ) ) {

				$exp_and_alt_list = array();
				$val = sanitize_text_field( $_POST['nelioab_form_env'] );
				try {
					$aux = explode( ',', $val );
					foreach ( $aux as $pair ) {
						try {
							$pair = explode( ':', $pair );
							$exp_and_alt_list[ sanitize_text_field( $pair[0] ) ] = absint( $pair[1] );
						} catch ( Exception $e ) {}
					}//end foreach
				} catch ( Exception $e ) {}

				update_post_meta( $order_id, '_nelioab_form_env', $exp_and_alt_list );
		...
		public function sync_order( $order_id, $old_status, $new_status ) {
			...
			if ( is_wp_error( $order ) || ! is_object( $order ) || ! method_exists( $order, 'get_items' ) ) {
				return;
			}//end if
			...
			foreach ( $env as $key_id => $alt ) {
				foreach ( $running_experiments as $exp ) {
					if ( $exp->get_key_id() != $key_id ) {
						continue;
					}
			...
			foreach ( $relevant_products as $product_id => $actual_product_id ) {
				...
				foreach ( $items as $item ) {
					if ( $this->is_purchased_product_equal_to( $item['product_id'], $product_id ) ) {
						$product_found = true;
						break;
					}//end if
				}
		...
		private function is_purchased_product_equal_to( $purchased_product_id, $expected_product_id ) {
			...
			if ( apply_filters( 'nelio_ab_testing_ignore_product_variations', false ) ) {
				return false;
			}//end if

			do {
				$product = get_product( $purchased_product_id );
		 ...
		 * @since PHPDOC
		 */
		public function fix_product_summary_featured_image( $value, $object_id, $meta_key ) {

			if ( '_thumbnail_id' == $meta_key ) {
				remove_filter( 'get_post_metadata', array( &$this, 'fix_product_summary_featured_image' ), 10 );
				...
				if ( $summary_data ) {
					...
					/** @var NelioABAlternative $alt */
					$alt = $summary_data['alt'];
					$new_value = $alt->get_value();

					if ( is_array( $new_value ) && isset( $new_value['image_id'] ) ) {
						$value = $new_value['image_id'];
					}//end if

				}//end if

				add_filter( 'get_post_metadata', array( &$this, 'fix_product_summary_featured_image' ), 10, 3 );
			}//end if

			return $value;

		}//end fix_product_summary_featured_image()
		...
		public function print_list_of_applied_product_summaries() {
			...
			<?php
			if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) { ?>
			// TODO: print it. NelioAB.helpers.sendNewHeadlineViews();
			<?php
			} ?>
			/* ]]> */
			</script><?php
