...
if ( !class_exists( 'NelioABProductSummaryAltExpEditionPageController' ) ) {
	...
	class NelioABProductSummaryAltExpEditionPageController extends NelioABAltExpSuperController {
		...
		protected function do_build() {
			$title = __( 'Edit Product Summary Experiment', 'nelio-ab-testing' );
			...
			if ( isset( $_REQUEST['post-id'] ) ) {
				$post_id = absint( $_REQUEST['post-id'] );
			}//end if

			$exp_type = false;
			if ( isset( $_REQUEST['experiment-type'] ) ) {
				$exp_type = absint( $_REQUEST['experiment-type'] );
			}//end if

			if ( $post_id && NelioABExperiment::WC_PRODUCT_SUMMARY_ALT_EXP == $exp_type ) {
				$experiment->set_original( $post_id );
			}//end if
			...
			if ( count( $list_of_posts ) == 0) {
				...
				$view = new NelioABMessagePage(
					__( 'There are no products available.', 'nelio-ab-testing' ),
					__( 'Please, create one and try again.', 'nelio-ab-testing' ) );
				return $view;
			}
		...
		public function build_experiment_from_post_data() {
			...
			$exp_original = false;
			if ( isset( $_REQUEST['exp_original'] ) ) {
				$exp_original = absint( $_REQUEST['exp_original'] );
			}//end if

			$exp = new NelioABProductSummaryAlternativeExperiment( $exp_id );
			$exp->set_original( $exp_original );
