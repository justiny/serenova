		...
		protected function do_build() {
			...
			if ( count( $themes ) < 2 ) {
				...
				$view = new NelioABMessagePage(
					__( 'There is only one theme available', 'nelio-ab-testing' ),
					__( 'Please, install one or more themes to create an experiment of this type.', 'nelio-ab-testing' ) );
				return $view;
			}
			...
			if ( isset( $_POST['nelioab_selected_themes'] ) ) {
				$selected_themes = json_decode( sanitize_text_field( stripslashes( $_POST['nelioab_selected_themes'] ) ) );
