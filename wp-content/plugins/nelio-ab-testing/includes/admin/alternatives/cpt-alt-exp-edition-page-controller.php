...
if ( !class_exists( 'NelioABCptAltExpEditionPageController' ) ) {
	...
	class NelioABCptAltExpEditionPageController extends NelioABAltExpSuperController {
		...
		protected function do_build() {
			...
			$title = __( 'Edit Custom Post Type Experiment', 'nelio-ab-testing' );
			...
			if ( isset( $_GET['post-id'] ) ) {
				$experiment->set_original( absint( $_GET['post-id'] ) );
			}//end if
			...
			if ( $alt_type == NelioABExperiment::CPT_ALT_EXP && count( $post_types ) == 0 ) {
				...
				$view = new NelioABMessagePage(
					__( 'There are no custom post types available.', 'nelio-ab-testing' ),
					__( 'You have to create a public custom post type and publish some custom posts first to use this type of experiment.', 'nelio-ab-testing' ) );
				return $view;
			}
			...
			if ( $alt_type == NelioABExperiment::CPT_ALT_EXP && !$found ) {
				require_once( NELIOAB_ADMIN_DIR . '/views/errors/message-page.php' );
				$view = new NelioABMessagePage(
					__( 'There are no custom posts available.', 'nelio-ab-testing' ),
					__( 'Please, create one custom post and try again.', 'nelio-ab-testing' ) );
				return $view;
			}
		...
		public function edit_alternative_content() {
			...
			// change).
			$alt_id = intval( $_POST['content_to_edit'] );
			$alt_to_edit = false;
		...
		public function build_experiment_from_post_data() {
			...
			if ( isset( $_POST['custom_post_types'] ) ) {
				$exp->set_post_type( sanitize_text_field( $_POST['custom_post_types'] ) );
			}
