...
if ( !class_exists( 'NelioABPostAltExpEditionPageController' ) ) {
	...
	class NelioABPostAltExpEditionPageController extends NelioABAltExpSuperController {
		...
		protected function do_build() {
			...
			$title = __( 'Edit Page Experiment', 'nelio-ab-testing' );
			if ( isset( $_GET['experiment-type'] ) && absint( $_GET['experiment-type'] ) == NelioABExperiment::POST_ALT_EXP ) {
				$alt_type = NelioABExperiment::POST_ALT_EXP;
				$title = __( 'Edit Post Experiment', 'nelio-ab-testing' );
			}
			...
			// ----------------------------------------------
			if ( isset( $_GET['post-id'] ) && $alt_type == NelioABExperiment::POST_ALT_EXP ) {
				$experiment->set_original( intval( $_GET['post-id'] ) );
			} elseif ( isset( $_GET['page-id'] ) && $alt_type == NelioABExperiment::PAGE_ALT_EXP ) {
				$experiment->set_original( intval( $_GET['page-id'] ) );
			}//end if
			...
			if ( $alt_type == NelioABExperiment::PAGE_ALT_EXP && count( $list_of_pages ) == 0 ) {
				...
				$view = new NelioABMessagePage(
					sprintf(
						__( 'There are no pages available.<br/><br/><a class="button button-primary" href="%s">Create one now.</a>', 'nelio-ab-testing' ),
			...
			if ( $alt_type == NelioABExperiment::POST_ALT_EXP && count( $list_of_posts ) == 0 ) {
				...
				$view = new NelioABMessagePage(
					sprintf(
						__( 'There are no posts available.<br/><br/><a class="button button-primary" href="%s">Create one now</a>', 'nelio-ab-testing' ),
		...
		public function edit_alternative_content() {
			...
			$url = add_query_arg( 'post', $alt_to_edit->get_value(), admin_url( 'post.php?action=edit' ) );
		...
		public function build_experiment_from_post_data() {
			...
			$exp = new NelioABPostAlternativeExperiment( absint( $_POST['exp_id'] ) );
			$exp->set_original( absint( $_POST['exp_original'] ) );
