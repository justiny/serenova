...
if ( !class_exists( 'NelioABHeatmapExpEditionPageController' ) ) {
	...
	class NelioABHeatmapExpEditionPageController {
		...
		protected function do_build() {
			$title = __( 'Edit Heatmap Experiment', 'nelio-ab-testing' );
			...
			if ( isset( $_GET['post-id'] ) ) {
				$experiment->set_post_id( intval( $_GET['post-id'] ) );
			} elseif ( isset( $_GET['page-id'] ) ) {
				$experiment->set_post_id( intval( $_GET['page-id'] ) );
			}//end if
			...
			if ( count( $list_of_pages ) + count( $list_of_posts ) == 0) {
				...
				$view = new NelioABErrorPage(
					__( 'There are no pages nor posts available.', 'nelio-ab-testing' ),
					__( 'Please, create some pages or posts and then try again.', 'nelio-ab-testing' ));
				return $view;
			}
		...
		public function print_ajax_errors() {
			...
			$msg = '<p>';
			$msg .= __( 'The following errors have been encountered:', 'nelio-ab-testing' );
			$msg .= '</p><ul>';
		...
		public function build_experiment_from_post_data() {
			...
			$exp = new NelioABHeatmapExperiment( absint( $_POST['exp_id'] ) );

			$exp->set_name( sanitize_text_field( stripslashes( $_POST['exp_name'] ) ) );
			$exp->set_description( sanitize_text_field( stripslashes( $_POST['exp_descr'] ) ) );
			$exp->set_post_id( intval( $_POST['exp_post_id'] ) );
