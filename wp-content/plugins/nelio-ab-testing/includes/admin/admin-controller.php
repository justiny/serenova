...
if ( !class_exists( 'NelioABAdminController' ) ) {
	...
	class NelioABAdminController {
		...
		public function __construct() {
			...
			if ( 'nelio-ab-testing' == $plugin && 'clean-and-deactivate' == $action ) {
		...
		protected function process_special_pages() {
			...
			switch ( sanitize_text_field( $_GET['nelioab-page'] ) ) {

				case 'save-css':
					$css = sanitize_text_field( $_POST['content'] );
					update_option( 'nelioab_temp_css', $css );

					$url = get_option('home');
					$url = add_query_arg( 'nelioab_preview_css', 'true', $url );
					if ( isset( $_GET['p'] ) ) {
						$url = add_query_arg( 'p', absint( $_GET['p'] ), $url );
					}//end if
					header( "Location: $url" );
					die();
		...
		public function init() {
			...
			require_once( NELIOAB_ADMIN_DIR . '/privacy.php' );
			NelioAB_Privacy::init();
		...
		public function admin_footer_text( $footer_text ) {
			...
			if ( ! get_option( 'nelioab_admin_footer_text_rated' ) ) {
				$footer_text = sprintf( __( 'If you like <strong>Nelio A/B Testing</strong> please leave us a %s&#9733;&#9733;&#9733;&#9733;&#9733;%s rating. A huge thank you from Nelio in advance!', 'nelio-ab-testing' ), '<a href="https://wordpress.org/support/view/plugin-reviews/nelio-ab-testing?filter=5#postform" target="_blank" class="nelioab-rating-link" data-rated="' . __( 'Thanks :)', 'nelio-ab-testing' ) . '">', '</a>' );
				...
				$footer_text = __( 'Thank you for using <a href="https://neliosoftware.com/testing/?plugin=footer-notice" target="_blank">Nelio A/B Testing.</a>', 'nelio-ab-testing' );
			}
		...
		public function add_gutenberg_styles() {
			...
			// Check whether there is a post_id set. If there is not any, we can quit.
			...
			require_once( NELIOAB_UTILS_DIR . '/wp-helper.php' );
			wp_register_style( 'nelioab_gutenberg_css',
				nelioab_admin_asset_link( '/css/nelioab-gutenberg.min.css' ),
				array(), NELIOAB_PLUGIN_VERSION );
			wp_enqueue_style( 'nelioab_gutenberg_css' );
		}
		...
		public function load_custom_style_and_scripts() {
			...
			if ( $this->is_page( array( 'exp-progress', 'nelioab-dashboard' ) ) ) {
				wp_enqueue_script( 'nelioab_chartjs',
					nelioab_admin_asset_link( '/js/chartjs.min.js' ),
					array(), NELIOAB_PLUGIN_VERSION );
			...
			if ( $promos_enabled_in_plan && $last_dismissal + WEEK_IN_SECONDS < time() && strpos( $page, 'nelioab-' ) == 0 ) {
		...
		private function do_check_is_page( $expected_page ) {
			...
			if ( strpos( $expected_page, 'nelioab' ) === 0 && $expected_page == $page ) {
				return true;
			}//end if
			...
			switch( $expected_page ) {
				...
				case 'edit-exp':
					if ( 'nelioab-experiments' == $page &&
					     isset( $_GET['action'] ) && 'edit' == sanitize_text_field( $_GET['action'] ) )
						return true;
					break;

				case 'exp-progress':
					if ( 'nelioab-experiments' == $page &&
					     isset( $_GET['action'] ) && 'progress' == sanitize_text_field( $_GET['action'] ) )
						return true;
					break;

				case 'add-headline-exp':
					if ( 'nelioab-add-experiment' == $page &&
						isset( $_GET['experiment-type'] ) && NelioABExperiment::HEADLINE_ALT_EXP == absint( $_GET['experiment-type'] ) )
						return true;
					break;
				...
				case 'edit-headline-exp':
					if ( 'nelioab-experiments' == $page &&
						isset( $_GET['action'] ) && 'edit' == sanitize_text_field( $_GET['action'] ) &&
						isset( $_GET['exp_type'] ) && NelioABExperiment::HEADLINE_ALT_EXP == absint( $_GET['exp_type'] ) )
						return true;
					break;
				...
				case 'add-product-summary-exp':
					if ( 'nelioab-add-experiment' == $page &&
						isset( $_GET['experiment-type'] ) && NelioABExperiment::WC_PRODUCT_SUMMARY_ALT_EXP == absint( $_GET['experiment-type'] ) )
						return true;
					break;
				...
				case 'edit-product-summary-exp':
					if ( 'nelioab-experiments' == $page &&
						isset( $_GET['action'] ) && 'edit' == sanitize_text_field( $_GET['action'] ) &&
						isset( $_GET['exp_type'] ) && NelioABExperiment::WC_PRODUCT_SUMMARY_ALT_EXP == absint( $_GET['exp_type'] ) )
						return true;
					break;
		...
		public function exclude_alternative_posts_and_pages( $query ) {
			...
			if ( $query->is_main_query() ||
			     ( isset( $_POST['action'] ) && 'menu-quick-search' == sanitize_text_field( $_POST['action'] ) ) ) {
				$alt_ids = array();
		...
		public function generate_html_content() {
			if ( isset( $_POST['classfi'] ) && isset( $_POST['classname'] ) ) {
				$file  = realpath( sanitize_text_field( $_POST['classfi'] ) );
				$class = sanitize_text_field( $_POST['classname'] );
		...
		public function add_plugin_action_links( $links, $file ) {
			if ( $file == plugin_basename( NELIOAB_PLUGIN_ID ) && is_plugin_active( $file ) ) {
				...
				$dashboard_link .= '      {';
				$dashboard_link .= '        text: "' . __( 'Cancel', 'nelio-ab-testing' ) .'",';
				$dashboard_link .= '        click: function() {';
				$dashboard_link .= '          jQuery(this).dialog("close");';
				$dashboard_link .= '        }';
				$dashboard_link .= '      },';
				$dashboard_link .= '      {';
				$dashboard_link .= '        text: "' . __( 'Clean', 'nelio-ab-testing' ) . '",';
				...
				$dashboard_link = sprintf( $dashboard_link,
						esc_html( __( 'Clean and Deactivate', 'nelio-ab-testing' ) ),
						esc_html( __( 'Warning!', 'nelio-ab-testing' ) ),
						esc_html( __( 'You are about to clean all your A/B testing data. This operation cannot be undone. Are you sure you want to continue?', 'nelio-ab-testing' ) ),
						wp_nonce_url(
		...
		public function create_nelioab_admin_pages() {
			...
			add_submenu_page( $nelioab_menu,
				__( 'Experiments', 'nelio-ab-testing' ),
				__( 'Experiments', 'nelio-ab-testing' ),
				'edit_posts',
				'nelioab-experiments',
				$page_to_build );
			...
			add_submenu_page( $nelioab_menu,
				__( 'Add Experiment', 'nelio-ab-testing' ),
				__( 'Add Experiment', 'nelio-ab-testing' ),
				'edit_posts',
			...
			if ( NelioABAccountSettings::is_using_free_trial() ) {
				$label = __( 'Free Trial', 'nelio-ab-testing' );
			} else {
				$label = __( 'My Account', 'nelio-ab-testing' );
			}
		...
		public function configure_edit_nelioab_alternative() {
			...
			foreach ( $post_types as $post_type ) {
				...
				add_meta_box(
					...
					__( 'Edition of Alternative\'s Content', 'nelio-ab-testing' ), // Box title
		...
		public function save_discussion_meta( $post_id ) {
			...
			$hide = false;
			if ( isset( $_REQUEST['nelioab_hide_discussion'] ) && 'on' === sanitize_text_field( $_REQUEST['nelioab_hide_discussion'] ) ) {
				$hide = true;
			}
			...
			if ( $hide ) {
				...
				$ori = get_post( get_post_meta( $post_id, '_nelioab_original_id', true ) );
				if ( $ori ) {
		...
		public function print_alternative_comments( $post ) {
			$checked = get_post_meta( $post->ID, '_nelioab_hide_discussion' );
			if ( count( $checked ) ) {
				$checked = ! empty( $checked[0] );
			} else {
				$checked = 'post' !== $post->post_type;
			}//end if
			?>
			...
			esc_html_e( 'Hide original discussion in this alternative.', 'nelio-ab-testing' );
		}
			...
			<div id="submitdiv"><?php
				...
				<script>
					...
					nelioab_style_metabox.innerHTML = "#save_nelioab_alternative_box h3.hndle { " +
						"color:<?php echo esc_attr( $colorscheme['foreground'] ); ?>; " +
						"background: <?php echo esc_attr( $colorscheme['focus'] ); ?>;" +
						"border: 1px solid <?php echo esc_attr( $colorscheme['focus'] ); ?>;" +
					"}" +
					...
					<div class="misc-pub-section" style="min-height:4em;">
						...
						<div style="float:right;margin-top:1em;margin-right:1em;">
							<div id="preview-action">
								<?php
									$type = 'post';
									$ori_id = get_post_meta( $the_post_id, '_nelioab_original_id', true );
									...
									$preview_link = get_permalink( $the_post_id );
									$preview_link = esc_url( add_query_arg( 'preview', 'true', $preview_link ) );
									?>
								<a class="preview button" href="<?php echo esc_url( $preview_link ); ?>" target="<?php echo esc_attr( "wp-preview-$the_post_id" ); ?>" id="post-preview"><?php echo esc_html( $preview_button ); ?></a>
						...
						<ul style="margin-left:1.5em;">
							...
							switch( $exp_status ){
								...
								case NelioABExperiment::STATUS_READY:
									?><li><a href="<?php echo $url . '&action=edit&ctab=tab-alts&id=' . $exp_id . '&exp_type=' . NelioABExperiment::PAGE_OR_POST_ALT_EXP; ?>"><?php
										esc_html_e( 'Editing this experiment', 'nelio-ab-testing' );
									?></a></li><?php
									break;
								...
								case NelioABExperiment::STATUS_FINISHED:
									...
									'&exp_type=' . NelioABExperiment::PAGE_OR_POST_ALT_EXP; ?>"><?php
										esc_html_e( 'The results of the related experiment', 'nelio-ab-testing' );
									?></a></li><?php
									break;
							...
							<?php if( $exp_status == NelioABExperiment::STATUS_RUNNING ) { ?>
								<li><a href="<?php echo admin_url( 'admin.php?page=nelioab-dashboard' ); ?>"><?php
									esc_html_e( 'The Dashboard', 'nelio-ab-testing' );
								?></a></li>
							<?php } ?>
						</ul>
					</div>
