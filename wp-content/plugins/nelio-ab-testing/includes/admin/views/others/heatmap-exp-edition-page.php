...
if ( !class_exists( 'NelioABHeatmapExpEditionPage' ) ) {
	...
	class NelioABHeatmapExpEditionPage extends NelioABAdminAjaxPage {
		...
		protected function do_render() { ?>
			<form class="nelio-exp-form" id="<?php echo esc_attr( $this->get_form_name() ); ?>" method="post" style="max-width:750px;">
				...
				<input type="hidden" name="nelioab_exp_type" value="<?php echo esc_attr( NelioABExperiment::HEATMAP_EXP ); ?>" />
				...
				<input type="hidden" name="other_names" id="other_names" value="<?php
					echo esc_attr( json_encode( $this->other_names ) );
				?>" />
				<?php
				$this->make_section(
		...
		protected function print_validator_js() { ?>
			...
			function submitAndRedirect(action,force) {
				...
				jQuery.post(
					location.href,
					jQuery(<?php echo json_encode( "#$this->form_name" ); ?>).serialize()
		...
		protected function get_basic_info_elements() {
			return array(
				...
				array (
					'label'     => esc_html__( 'Page, Post, or Custom Post', 'nelio-ab-testing' ),
					'id'        => 'exp_original',
		...
		public function print_page_buttons() {
			echo $this->make_js_button(
					_x( 'Update', 'action', 'nelio-ab-testing' ),
					'javascript:submitAndRedirect("validate",false)',
					false, true
				);
			echo $this->make_js_button(
					_x( 'Cancel', 'action', 'nelio-ab-testing' ),
		...
		public function print_name_field() { ?>
			<input name="exp_name" type="text" id="exp_name" maxlength="250"
				class="regular-text" value="<?php echo esc_attr( $this->basic_info['name'] ); ?>" />
		...
		public function print_descr_field() { ?>
			<textarea id="exp_descr" style="width:300px;" maxlength="450"
				name="exp_descr" cols="45" rows="3"><?php echo esc_attr( $this->basic_info['description'] ); ?></textarea>
			<span class="description" style="display:block;"><?php
