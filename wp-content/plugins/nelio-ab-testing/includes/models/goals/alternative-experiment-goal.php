...
if ( ! class_exists( 'NelioABAltExpGoal' ) ) {
	...
	class NelioABAltExpGoal extends NelioABGoal {
		...
		public function get_results() {
			...
			if ( $goal_results ) {
				$json_data = json_decode( $goal_results['data'], true );
			}//end if
			...
			$alt_res->set_name( __( 'Original', 'nelio-ab-testing' ) );
			...
			if ( is_array( $json_data['gTests'] ) ) {
				foreach ( $json_data['gTests'] as $stats ) {
					...
					$g->set_min_name( __( 'Unknown', 'nelio-ab-testing' ) );
					$g->set_max_name( __( 'Unknown', 'nelio-ab-testing' ) );
					$alts = $experiment->get_alternatives();
					for( $i = 0; $i < count( $alts ); ++$i ) {
						$alt = $alts[$i];
						$short_name = sprintf( __( 'Alternative %s', 'nelio-ab-testing' ), ( $i + 1 ) );
					...
					if ( $experiment->get_originals_id() == $min_ver )
						$g->set_min_name( __( 'Original', 'nelio-ab-testing' ) );
					if ( $experiment->get_originals_id() == $max_ver )
						$g->set_max_name( __( 'Original', 'nelio-ab-testing' ) );
		...
		public function sync() {
			...
			if ( $goal_results ) {
				...
				if ( isset( $goal_results['data'] ) ) {
					$json_data = json_decode( $goal_results['data'] );
			...
			try {
				...
				for ( $i = 0; $i < $num_of_goals; ++$i ) {
					...
					if ( $goal_result_set[ $i ]['id'] === $this->get_id() ) {
						...
						$goal_result_set[ $i ]['data'] = json_encode( $json_data );
						$found = true;
						break;
					}//end if
