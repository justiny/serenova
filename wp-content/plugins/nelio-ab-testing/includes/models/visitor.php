...
if ( !class_exists( 'NelioABVisitor' ) ) {
	...
	class NelioABVisitor {
		 ...
		 * @var array
		 */
		private static $is_first_load_done = false;
		 ...
		 * @var array
		 */
		private static $links_info = array(
			'nabc'   => false,
			'nabm'   => false,
			'nabt'   => false,
			'nabw'   => false,
		...
		public static function load() {
			add_action( 'setup_theme', array( 'NelioABVisitor', 'do_first_load' ), 1 );
		...
		public static function do_first_load() {
			...
			if ( isset( $_POST['nelioab_env'] ) ) {
				...
				$aux = $_POST['nelioab_env'];
				if ( ! is_array( $aux ) ) {
					$aux = array();
				}//end if

				foreach ( $aux as $key => $val ) {
					$exp_and_alt_list[ sanitize_text_field( $key ) ] = absint( $val );
				}//end if
				...
				if ( isset( $_GET['nabe'] ) ) {
					...
					$val = sanitize_text_field( $_GET['nabe'] );
					try {
						$aux = explode( ',', $val );
						foreach ( $aux as $pair ) {
							try {
				...
				if ( isset( $_GET['nabc'] ) ) {
					$exp = self::get_running_global_experiment( NelioABExperiment::CSS_ALT_EXP );
					if ( $exp ) {
						$exp_and_alt_list[ $exp->get_key_id() ] = absint( $_GET['nabc'] );
					}//end if
				}//end if

				if ( isset( $_GET['nabm'] ) ) {
					$exp = self::get_running_global_experiment( NelioABExperiment::MENU_ALT_EXP );
					if ( $exp ) {
						$exp_and_alt_list[ $exp->get_key_id() ] = absint( $_GET['nabm'] );
					}//end if
				}//end if

				if ( isset( $_GET['nabt'] ) ) {
					$exp = self::get_running_global_experiment( NelioABExperiment::THEME_ALT_EXP );
					if ( $exp ) {
						$exp_and_alt_list[ $exp->get_key_id() ] = absint( $_GET['nabt'] );
					}//end if
				}//end if

				if ( isset( $_GET['nabw'] ) ) {
					$exp = self::get_running_global_experiment( NelioABExperiment::WIDGET_ALT_EXP );
					if ( $exp ) {
						$exp_and_alt_list[ $exp->get_key_id() ] = absint( $_GET['nabw'] );
					}//end if
				}//end if
		...
		private static function get_running_global_experiment( $type ) {
		...
		}//end get_running_global_experiment()
