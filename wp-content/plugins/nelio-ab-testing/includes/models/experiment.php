...
if ( !class_exists( 'NelioABExperiment' ) ) {
	...
	abstract class NelioABExperiment {
		 ...
		 * It returns the ID of this experiment.
		 *
		 * @return string the ID of this experiment.
		 *
		 * @since 4.3.2
		 */
		...
		public function start() {
			...
			try {
				...
				foreach ( $running_exps as $exp ) {
					array_push( $running_ids, $exp->get_key_id() );
				}//end foreach
				...
				$params = array( 'body' => array(
					...
					'runningExps'        => json_encode( $running_ids ),
		...
		public static function get_label_for_status( $status ) {
			switch ( $status ) {
			case NelioABExperiment::STATUS_DRAFT:
					return __( 'Draft', 'nelio-ab-testing' );
			case NelioABExperiment::STATUS_PAUSED:
					return __( 'Paused', 'nelio-ab-testing' );
				case NelioABExperiment::STATUS_READY:
					return __( 'Prepared', 'nelio-ab-testing' );
				case NelioABExperiment::STATUS_FINISHED:
					return __( 'Finished', 'nelio-ab-testing' );
				case NelioABExperiment::STATUS_RUNNING:
					return __( 'Running', 'nelio-ab-testing' );
				case NelioABExperiment::STATUS_SCHEDULED:
					return __( 'Scheduled', 'nelio-ab-testing' );
				case NelioABExperiment::STATUS_TRASH:
					return __( 'Trash' );
				default:
					return __( 'Unknown Status', 'nelio-ab-testing' );
			}
		}
