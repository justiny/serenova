			...
			foreach ( $running_exps as $running_exp ) {
				...
				switch ( $running_exp->get_type() ) {
					...
					case NelioABExperiment::THEME_ALT_EXP:
						$err_str = sprintf(
							esc_html__( 'The experiment cannot be started, because there is a theme experiment running. Please, stop the experiment named «%s» before starting the new one.', 'nelio-ab-testing' ),
					...
					case NelioABExperiment::CSS_ALT_EXP:
						...
						foreach( $this_exp_origins as $origin_id ) {
							if ( in_array( $origin_id, $running_exp->get_origins() ) ) {
								$err_str = sprintf(
									esc_html__( 'The experiment cannot be started, because there is a CSS experiment running. Please, stop the experiment named «%s» before starting the new one.', 'nelio-ab-testing' ),
									esc_html( $running_exp->get_name() ) );
								throw new Exception( $err_str, NelioABErrCodes::EXPERIMENT_CANNOT_BE_STARTED );
							}
						}
					...
					case NelioABExperiment::WIDGET_ALT_EXP:
						...
						foreach( $this_exp_origins as $origin_id ) {
							if ( in_array( $origin_id, $running_exp->get_origins() ) ) {
								$err_str = sprintf(
									esc_html__( 'The experiment cannot be started, because there is a Widget experiment running. Please, stop the experiment named «%s» before starting the new one.', 'nelio-ab-testing' ),
									esc_html( $running_exp->get_name() ) );
								throw new Exception( $err_str, NelioABErrCodes::EXPERIMENT_CANNOT_BE_STARTED );
							}
						}
					...
					case NelioABExperiment::MENU_ALT_EXP:
						...
						foreach( $this_exp_origins as $origin_id ) {
							if ( in_array( $origin_id, $running_exp->get_origins() ) ) {
								$err_str = sprintf(
									esc_html__( 'The experiment cannot be started, because there is a Menu experiment running. Please, stop the experiment named «%s» before starting the new one.', 'nelio-ab-testing' ),
									esc_html( $running_exp->get_name() ) );
								throw new Exception( $err_str, NelioABErrCodes::EXPERIMENT_CANNOT_BE_STARTED );
							}
						}
					...
					case NelioABExperiment::HEATMAP_EXP:
						if ( $this->get_type() != NelioABExperiment::WIDGET_ALT_EXP ) {
							$err_str = __( 'The experiment cannot be started, because there is one (or more) heatmap experiments running. Please make sure to stop any running heatmap experiment before starting the new one.', 'nelio-ab-testing' );
							throw new Exception( $err_str, NelioABErrCodes::EXPERIMENT_CANNOT_BE_STARTED );
						}
						break;
