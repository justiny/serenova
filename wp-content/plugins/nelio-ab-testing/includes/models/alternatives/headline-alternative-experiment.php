	...
	class NelioABHeadlineAlternativeExperiment extends NelioABAlternativeExperiment {
		...
		public function add_alternative( $alt ) {
			...
			foreach ( $this->get_alternatives() as $aux ) {
				...
				$value = $aux->get_value();
				if ( $value['id'] <= $fake_post_id )
					$fake_post_id = $value['id'] - 1;
			}
			$val = $alt->get_value();
			$val['id'] = $fake_post_id;
			$alt->set_value( $val );
		...
		public function pre_start() {
			...
			if ( get_post_status( $this->get_originals_id() ) == 'draft' ) {
				if ( get_post_type( $this->get_originals_id() ) == 'page' ) {
					$err_str = __( 'The experiment cannot be started, because the tested page is a draft.', 'nelio-ab-testing' );
				} else {
					$err_str = __( 'The experiment cannot be started, because the tested post is a draft.', 'nelio-ab-testing' );
				}
			...
			foreach ( $running_exps as $running_exp ) {
				...
				if ( $running_exp->get_originals_id() == $this->get_originals_id() ) {
					if ( $running_exp->get_type() == NelioABExperiment::PAGE_ALT_EXP ) {
						$err_str = sprintf(
							esc_html__( 'The experiment cannot be started, because there is another experiment running that is testing the same page. Please, stop the experiment named «%s» before starting the new one.', 'nelio-ab-testing' ),
							esc_html( $running_exp->get_name() ) );
					}
					else if ( $running_exp->get_type() == NelioABExperiment::POST_ALT_EXP ) {
						$err_str = sprintf(
							esc_html__( 'The experiment cannot be started, because there is another experiment running that is testing the same post. Please, stop the experiment named «%s» before starting the new one.', 'nelio-ab-testing' ),
							esc_html( $running_exp->get_name() ) );
					}
					else if ( $running_exp->get_type() == NelioABExperiment::CPT_ALT_EXP ) {
						$err_str = sprintf(
							esc_html__( 'The experiment cannot be started, because there is another experiment running that is testing the same custom post. Please, stop the experiment named «%s» before starting the new one.', 'nelio-ab-testing' ),
							esc_html( $running_exp->get_name() ) );
					}
					else if ( $running_exp->get_type() == NelioABExperiment::HEADLINE_ALT_EXP ) {
						$err_str = sprintf(
							esc_html__( 'The experiment cannot be started, because there is another experiment that is testing the title of the same page. Please, stop the experiment named «%s» before starting the new one.', 'nelio-ab-testing' ),
							esc_html( $running_exp->get_name() ) );
					}
					else if ( $running_exp->get_type() == NelioABExperiment::WC_PRODUCT_SUMMARY_ALT_EXP ) {
						$err_str = sprintf(
							esc_html__( 'The experiment cannot be started, because there is another experiment that is testing the same product. Please, stop «%s» before starting the new experiment.', 'nelio-ab-testing' ),
							esc_html( $running_exp->get_name() ) );
					}
					throw new Exception( $err_str, NelioABErrCodes::EXPERIMENT_CANNOT_BE_STARTED );
				}
