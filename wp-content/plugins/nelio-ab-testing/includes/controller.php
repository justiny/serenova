...
if ( !class_exists( 'NelioABController' ) ) {
	...
	class NelioABController {
		...
		public function init() {
			...
			// Load Alternative Experiment Controller
			...
			// CoAuthors Plus Support: making sure that this plugin doesn't throw a notice when checking local experiments
			require_once( NELIOAB_UTILS_DIR . '/coauthors-plus-support.php' );
			NelioABCoAuthorsPlus::hook_to_wordpress();

			if ( is_admin() && ! $this->is_doing_ajax_request() ) {
				return;
			}//end if
			...
			require_once( NELIOAB_UTILS_DIR . '/wp-helper.php' );
			if ( ! is_admin() && NelioABWpHelper::is_at_least_version( 3.4 ) ) {
		...
		public function add_js_for_compatibility() {
			...
			?><script type="text/javascript">NelioAB={checker:{generateAjaxParams:function(){return {};}}}</script>
<!-- This site uses Nelio A/B Testing <?php echo esc_html( NELIOAB_PLUGIN_VERSION ); ?> - https://neliosoftware.com/testing/ -->
<!-- @Webmaster, normally you will find Nelio's tracking scripts in the head section. However, admin users are excluded from tests (which means those scripts are not necessary). -->
<?php
		}
		...
		public function create_nelioab_admin_bar_menu() {
			...
			$wp_admin_bar->add_node( array(
				...
				'title' => __( 'Nelio A/B Testing', 'nelio-ab-testing' ),
			...
			$wp_admin_bar->add_node( array(
				...
				'title'  => __( 'Add Experiment', 'nelio-ab-testing' ),
			...
			if ( $is_post ) {
				...
				$wp_admin_bar->add_node(array(
					...
					'title' => __( 'Create Headline Test for this Post', 'nelio-ab-testing' ),
				...
				$wp_admin_bar->add_node(array(
					...
					'title' => __( 'Create A/B Test for this Post', 'nelio-ab-testing' ),
			...
			if ( $is_page ) {
				...
				$wp_admin_bar->add_node(array(
					...
					'title' => __( 'Create A/B Test for this Page', 'nelio-ab-testing' ),
			...
			if ( $is_post || $is_page ) {
				...
				$wp_admin_bar->add_node( array(
					...
					'title'  => __( 'Create Heatmap Experiment', 'nelio-ab-testing' ),
		...
		public function create_nelioab_admin_bar_quickexp_option() {
			...
			if ( $is_post ) {
				$wp_admin_bar->add_node(array(
					...
					'title' => __( 'Create Headline Experiment', 'nelio-ab-testing' ),
					'href' => admin_url( 'admin.php?page=nelioab-add-experiment&experiment-type=' . NelioABExperiment::HEADLINE_ALT_EXP . $current_element ),
				));
			}
			...
			if ( $is_page ) {
				$wp_admin_bar->add_node(array(
					'id' => 'nelioab_admin_bar_quick_menu',
					'title' => __( 'Create A/B Experiment', 'nelio-ab-testing' ),
					'href' => admin_url( 'admin.php?page=nelioab-add-experiment&experiment-type=' . NelioABExperiment::PAGE_ALT_EXP . $current_element ),
				));
			}
		}
		...
		public function is_doing_ajax_request() {
	...
	if ( $nelioab_controller->is_doing_ajax_request() ) {
