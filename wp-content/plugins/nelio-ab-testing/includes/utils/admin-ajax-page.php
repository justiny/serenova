...
if ( !class_exists( 'NelioABAdminAjaxPage' ) ) {
	...
	abstract class NelioABAdminAjaxPage extends NelioABAdminPage {
		...
		public function render() { ?>
			<script type="text/javascript" src="<?php echo esc_url( nelioab_admin_asset_link( '/js/tablesorter.min.js' ) ); ?>"></script><?php
			...
			<div class="wrap">
				...
				<div id="ajax-loader" style="text-align:center;<?php echo $is_data_pending_loader; ?>">
					...
					<h2 style="color:#555;margin:0;padding:0;"><?php esc_html_e( 'Loading...', 'nelio-ab-testing' ); ?></h2>
					<p id="ajax-loader-label1" style="color:#777;margin:0;padding:0;"><?php esc_html_e( 'Please, wait a moment.', 'nelio-ab-testing' ); ?></p>
					<p id="ajax-loader-label2" style="color:#777;margin:0;padding:0;display:none;"><?php esc_html_e( 'Keep waiting...', 'nelio-ab-testing' ); ?></p>
					<p id="ajax-loader-label3" style="color:#777;margin:0;padding:0;display:none;"><?php esc_html_e( 'Internet connection seems very slow.', 'nelio-ab-testing' ); ?></p>
				</div>
			...
			<script>
				...
				jQuery(document).ready(function() {
					...
					var data = <?php
						$aux = array();
						foreach ( $this->post_params as $value ) {
							$aux[ $value[0] ] = $value[1];
						}//end if
						$aux['action'] = 'nelioab_get_html_content';
						$aux['classfi'] = $this->controller_file;
						$aux['classname'] = $this->controller_class;
					...
					jQuery.ajax({
						...
						error: function() {
							...
							<?php
								echo json_encode( sprintf( '<img src="%s" alt="%s" />',
									esc_url( nelioab_asset_link( '/admin/images/error-icon.png' ) ),
									esc_attr__( 'Funny image to graphically notify of an error.', 'nelio-ab-testing' )
								) );
							?> +
							"<h2 style='color:#555;margin:0;padding:0;'><?php
								esc_html_e( 'Oops! There was an AJAX-related error.' );
							?></h2>" +
		...
		protected function make_form_javascript( $form_name, $hidden_action ) {
			return sprintf( ' onclick="%s" ',
				esc_attr( sprintf(
					'javascript:smoothTransitions();jQuery(%s).attr("value",%s);jQuery(%s).submit();',
