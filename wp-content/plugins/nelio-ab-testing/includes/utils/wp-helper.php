...
if ( !class_exists( 'NelioABWpHelper' ) ) {
	...
	abstract class NelioABWpHelper {
		...
		public static function copy_meta_info( $src_id, $dest_id, $copy_hidden = true ) {
			...
			global $wpdb;
			$metas = $wpdb->get_results( "SELECT * FROM $wpdb->postmeta WHERE post_id = $src_id" );
			if ( ! is_array( $metas ) ) {
				return;
			}//end if
			foreach ( $metas as $meta ) {
				...
				if ( strpos( $meta->meta_key, 'nelioab_' ) !== false ) {
					continue;
				}//end if

				if ( ! $copy_hidden && substr( $meta->meta_key, 0, 1 ) == '_' ) {
					continue;
				}//end if
		...
		public static function search_posts() {
			...
			if ( isset( $_POST['type'] ) ) {
				if ( is_array( $_POST['type'] ) ) {
					foreach ( $_POST['type'] as $type ) {
						array_push( $types, sanitize_text_field( $type ) );
					}//end foreach
				} else {
					$types = sanitize_text_field( $_POST['type'] );
				}//end if
			}//end if

			if ( 'nelioab-all-cpt' === $types ) {
				$types = array( 'post' );
				$post_types = get_post_types( array( 'public' => true, '_builtin' => false ), 'objects' );
				foreach ( $post_types as $pt ) {
					if ( 'page' === $pt->name ) continue;
					if ( 'product' === $pt->name ) continue;
					array_push( $types, $pt->name );
				}//end foreach
			...
			$status = 'publish';
			if ( isset( $_POST['drafts'] ) && 'show-drafts' == sanitize_text_field( $_POST['drafts'] ) )
			...
			$default_thumbnail = sprintf(
				...
				esc_attr__( 'No featured image available', 'nelio-ab-testing' )
			);
			...
			if ( in_array( 'nelioab-latest-posts', $types ) || false !== $default_id ) {
				$lp_title = __( 'Your latest posts', 'nelio-ab-testing' );
			...
			if ( ( in_array( 'nelioab-theme-landing-page', $types ) && NelioABSettings::does_theme_use_a_custom_landing_page() ) ||
				...
				$lp_title = __( 'Landing Page (Theme-based)', 'nelio-ab-testing' );
				if ( !$term || strpos( strtolower( $lp_title ), strtolower( $term ) ) !== false ) {
			...
			if ( $default_id !== false ) {
				...
				if ( $id == NelioABController::FRONT_PAGE__YOUR_LATEST_POSTS ) {
					wp_send_json( array( $latest_post_item ) );
				}

				if ( $id == NelioABController::FRONT_PAGE__THEME_BASED_LANDING ) {
					wp_send_json( array( $theme_based_landing_page ) );
				}
				...
				if ( $post ) {
					...
					wp_send_json( array( $item ) );
				}
			}

			$result  = array();
			$post_id = max( 0, intval( trim( $term ) ) );

			if ( $post_id ) {
				$post = get_post( $post_id );
				if ( $post && ! is_wp_error( $post ) && in_array( $post->post_type, $types, true ) ) {
					$thumbnail = get_the_post_thumbnail( $post->ID, 'thumbnail' );
					if ( $thumbnail === '' ) {
						$thumbnail = $default_thumbnail;
					}//end if
		...
		private static function fix_title( $title ) {
			...
			if ( strlen( $title ) == 0 )
				$title = __( '(no title)', 'nelio-ab-testing' );
			return $title;
		}
