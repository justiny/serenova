	...
	abstract class NelioABAdminTable extends WP_List_Table {
		...
		public function column_default( $item, $column_name){
			...
			try {
				...
				return sprintf( __( 'Error displaying column `%s\'.', 'nelio-ab-testing' ), $column_name );
			}
		}
		...
		public function display() {
			if ( $this->get_table_id() )
				echo '<span style="display:none;" id="' . esc_attr( 'span-' . $this->get_table_id() ) . '"></span>';
			parent::display();
			...
			if ( $id ) {
				...
				foreach ( $cols as $col => $val ) {
					if ( ! in_array( $col, $sort ) ) {
						$head[ $i ] = array( 'sorter' => false );
					}//end if
					++$i;
				}//end foreach
				?>
				<script>if (typeof jQuery !== 'undefined') {
					jQuery(<?php echo json_encode( $span_id ); ?>).nextAll("table").first().
						attr("id", <?php echo json_encode( $id ); ?>);
					jQuery(<?php echo json_encode( $span_id ); ?>).remove();
					jQuery(<?php echo json_encode( $table_id ); ?>).
						find("th").each(function() {
							jQuery(this).html(
								"<a><span>" + jQuery(this).text() + "</span>" +
								"<span class='sorting-indicator'></span></a>");
						});
					jQuery(<?php echo json_encode( $table_id ); ?>).first().
					tablesorter({
						headers: <?php echo json_encode( $head ); ?>,
						cssAsc: 'sorted desc',
						cssDesc: 'sorted asc',
				...
				<td colspan="<?php echo esc_attr( $this->get_column_count() ); ?>" class="colspanchange">
					...
					<p class="submit inline-edit-save">
						<a class="button button-primary save alignleft"
							...
							echo esc_html( $this->inline_save_button_label() ); ?></a>
