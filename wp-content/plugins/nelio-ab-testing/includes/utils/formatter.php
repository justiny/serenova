...
if ( !class_exists( 'NelioABFormatter' ) ) {
	...
	abstract class NelioABFormatter {
		 ...
		 * @since 1.0.10
		 */
		public static function format_date( $timestamp, $tz = false, $format = false ) {
			$aux = $timestamp;
			if ( !is_int( $aux ) ) {
				$aux = new DateTime( $timestamp);
				$aux = date_timestamp_get( $aux );
			}

			return NelioABFormatter::format_unix_timestamp( $aux, $tz, $format );
		}
		 ...
		 * @since 1.0.10
		 */
		public static function format_unix_timestamp( $timestamp, $tz = false, $format = false ) {
			if ( $tz === false ) {
		...
		public static function get_timelapse( $timestamp_start, $timestamp_end ) {
			...
			if ( $months > 0 ) {
				$result = sprintf(
					_n( '1 month', '%d months', $months, 'nelio-ab-testing' ),
					$months );
				if ( $days > 0 ) {
					$result .= ' ' . sprintf(
						_n( 'and 1 day', 'and %d days', $days, 'nelio-ab-testing' ),
						$days );
				}
			} else if ( $days > 0 ) {
				$result = sprintf(
					_n( '1 day', '%d days', $days, 'nelio-ab-testing' ),
					$days );
				if ( $hours > 0 ) {
					$result .= ' ' . sprintf(
						_n( 'and 1 hour', 'and %d hours', $hours, 'nelio-ab-testing' ),
						$hours );
				}

			} else if ( $hours > 0 ) {
				$result = sprintf(
					_n( '1 hour', '%d hours', $hours, 'nelio-ab-testing' ), $hours );
			} else {
				$result = __( 'Less than 1 hour', 'nelio-ab-testing' );
			}
