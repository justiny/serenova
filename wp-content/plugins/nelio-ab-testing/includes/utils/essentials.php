...
function nelioab_deactivate_plugin() {
	...
	if ( 'clean-and-deactivate' === $action ) {
...
function nelioab_add_cache_notice() {
	...
	<div class="updated">
		<p>
			<?php echo $message; ?>
			<a id="dismiss-nelioab-cache-notice" style="font-size:80%;" href="#"><?php esc_html_e( 'Dismiss' ); ?></a>
		</p>
...
/**
 * This function returns the URL of the plugin.
 *
 * @return string the URL of the plugin.
 *
 * @since 4.5.11
 */
function nelioab_url() {
	return preg_replace( '/^https?:/', '', plugins_url() . '/' . NELIOAB_PLUGIN_DIR_NAME );
}
...
function nelioab_asset_link( $resource, $include_version = true ) {
	$link = nelioab_url() . '/assets' . $resource;
	if ( $include_version )
		$link = add_query_arg( array( 'ver' => NELIOAB_PLUGIN_VERSION ), $link );
	return $link;
}
